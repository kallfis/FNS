...
function setActiveNav() {
    ...
    navLinks.forEach(link => {
        ...
        // Убираем лишние символы для сравнения
    ...
    if (document.querySelector('.team-container')) {
        ...
        const socialPlates = document.querySelectorAll('.social-plate');
        ...
        socialPlates.forEach((plate, index) => {
            plate.style.opacity = '0';
            plate.style.transform = 'translateY(10px)';
            plate.style.transition = `opacity 0.5s ease ${0.3 + index * 0.1}s, transform 0.5s ease ${0.3 + index * 0.1}s`;
...
document.querySelectorAll('nav a').forEach(link => {
    if (link.href && !link.href.includes('#')) {
        link.addEventListener('click', function(e) {
            // Если кликаем по уже активной ссылке
