...
// Проверка перекрытия логотипа и меню
function checkLogoOverlap() {
    ...
    // Если логотип перекрывает навигацию, скрываем текст
    if (logoRect.right > navRect.left) {
        document.querySelector('.logo-text').style.display = 'none';
        document.querySelector('.logo-container').style.left = '10px';
    } else {
        document.querySelector('.logo-text').style.display = 'flex';
    }
}
